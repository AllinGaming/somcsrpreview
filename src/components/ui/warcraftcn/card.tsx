  ...
  return (
    <div
      ...
      className={cn(
        ...
        "border-16 px-5 [border-image-slice:16_fill]",
        "data-[size=sm]:border-12 data-[size=sm]:p-3 data-[size=sm]:py-2 data-[size=sm]:[border-image-slice:12_fill]",
        className
      )}
      {...props}
    />
...
function CardHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "gap-1 pt-4 rounded-t-xl px-4 group-data-[size=sm]/card:px-3 group/card-header @container/card-header grid auto-rows-min items-start has-data-[slot=card-action]:grid-cols-[1fr_auto] has-data-[slot=card-description]:grid-rows-[auto_auto]",
        className
      )}
      {...props}
    />
